...
import org.openehealth.ipf.tools.manager.flowmanager.FlowManagerEvent;
...
public class FlowManagerRepositorylImpl extends Observable implements
    ...
    public void unRegisterFlowManager(
        ...
        if (connectionConfiguration != null) {
            Activator.getDefault().info(
                    "Unregistering flow manager adapter for connection "
        ...
        synchronized (this) {
            if (this.flowManagers.containsKey(connectionConfiguration)) {
                ...
                Activator
                        .getDefault()
                        .error(
                                "isFlowManagerRegistered called for an offline connection!");
            }
            return false;
        }
    }
        ...
        synchronized (this) {
            ...
            if (fm != null) {
                ...
                Activator.getDefault().error(
                        "Flow Manager not found! Cannot set flow Filtering.");
            }
        }
        flowManagerStateChanged(event);
    }
    ...
    protected void setApplication(
        ...
        synchronized (this) {
            IFlowManagerMBean fm = flowManagers.get(connectionConfiguration);
            Activator.getDefault().info(
                    "Setting application for connection "
            ...
            if (fm != null) {
                ...
                Activator.getDefault().error(
                        "Flow Manager not found! Cannot set max application.");
                event = null;
            }
        }
        flowManagerStateChanged(event);
        ...
        synchronized (this) {
            ...
            if (fm != null) {
                ...
                event = new FlowManagerEvent(connectionConfiguration,
                        FlowManagerEvent.FLOWMANAGER_ATTRIBUTE_CHANGED);
                Activator.getDefault().info(
                        "Settings max flows for connection "
                                + connectionConfiguration);
            } else {
                event = null;
                Activator.getDefault().error(
                        "Flow Manager not found! Cannot set max flows.");
            }
        }
        flowManagerStateChanged(event);
    }
    ...
    public String getApplication(
        ...
        try {
            ...
            Activator.getDefault().error("The connection was broken!" + t);
    ...
    /**
     * Replays the given flows. After the replay, makes a search with the given
     * criteria.
     * 
     * @param connectionConfiguration
     *            the target connection configuration.
     * @param flows
     *            the flows to be replayed
     * @param searchCriteria
     * @throws InterruptedException
     * @throws ParseException
     *             if the dates in the serach criteria are invalid
     */
    ...
    public void searchFlows(IConnectionConfiguration connectionConfiguration,
        ...
        synchronized (this) {
            // search
            flows = searchFlowsInternal(connectionConfiguration, criteria);
    ...
    private void registerFlows(
        ...
        Activator.getDefault().info(
                "Notifying observers for registered flows for connection "
                        + connectionConfiguration);
        if (flows.containsKey(connectionConfiguration)) {
            flows.remove(connectionConfiguration);
    ...
    private List<FlowInfo> searchFlowsInternal(
        ...
        Activator.getDefault().error(
                "Returning no flows, flow manager is unregistered!");
    ...
    private List<FlowInfo> doFlowManagerFind(IFlowManagerMBean fm,
        ...
        if (criteria == null) {
            Activator.getDefault().info("Criteria are not restricted.");
    ...
    protected IFlowManagerMBean loadFlowManager(
        ...
        if (connectionConfiguration == null
                || !this.jMXConnectionManager.isOpen(connectionConfiguration)) {
            return null;
        }
        ...
        try {
            ...
            Activator.getDefault().error("Unable to load flow manager", ioe);
            return null;
        } catch (MalformedObjectNameException e) {
            Activator.getDefault().error("Unable to load flow manager", e);
            return null;
    ...
    protected void handleUncheckedException(
        ...
        if (cause == null) {
            Activator.getDefault().error(
                    "Unknown Exception detected for connection"
                            + connectionConfiguration, cause);
        }
        if (cause instanceof ConnectException) {
            Activator.getDefault().info(
                    "ConnectException detected for connection "
                            + connectionConfiguration);
            this.unRegisterFlowManager(connectionConfiguration);
        } else if (cause instanceof IllegalArgumentException) {
            Activator.getDefault().info(
                    "IllegalArgumentException detected for connection "
                            + connectionConfiguration, cause);
