 ...
 */
package org.openehealth.ipf.tools.manager.jmxexplorer.ui.tree.builder;
...
import org.openehealth.ipf.tools.manager.connection.ui.tree.InitializableNodeAbstract;
...
import org.openehealth.ipf.tools.manager.jmxexplorer.ui.tree.MBeanNotificationNode;
import org.openehealth.ipf.tools.manager.jmxexplorer.ui.tree.MBeanNotificationsGroupNode;
...
import org.openehealth.ipf.tools.manager.jmxexplorer.ui.tree.builder.strategy.NodeOrderStrategyAbstract;
        ...
        if (factorDepth == propertyNames.size()) {
            ...
            try {
                ...
                Activator.getDefault().error(
                        "Unable to create the connection node", infe);
                mbeanNode = new MBeanNode(parentNode.getName(), objectName,
                        new EmptyMBeanInfo(parentNode.getName(), infe
                                .getLocalizedMessage()));
            } catch (IntrospectionException ie) {
                Activator.getDefault().error(
                        "Unable to create the connection node", ie);
                // add an empty node!
                mbeanNode = new MBeanNode(parentNode.getName(), objectName,
