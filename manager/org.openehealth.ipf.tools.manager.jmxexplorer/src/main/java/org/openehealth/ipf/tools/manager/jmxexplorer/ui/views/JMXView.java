...
import org.openehealth.ipf.tools.manager.connection.ui.utils.jobs.JobUtils;
...
import org.openehealth.ipf.tools.manager.jmxexplorer.ui.tree.JMXConnectionNode;
import org.openehealth.ipf.tools.manager.jmxexplorer.ui.tree.JMXConnectionTree;
...
public class JMXView extends ViewPart implements Observer, MouseListener {
    ...
    public void mouseDoubleClick(MouseEvent e) {
        try {
            ...
            services
                    .executeCommand(
                            "org.openehealth.ipf.tools.manager.jmxexplorer.ui.open.jmxeditor",
                            null);
        } catch (NotHandledException ex) {
            Activator.getDefault().error(
                    "The double click on a connection is not handled", ex);
        } catch (ExecutionException ex) {
            Activator.getDefault().error("Double click execution exception: ",
                    ex);
        } catch (NotDefinedException ex) {
            Activator.getDefault().error("Not defined double click action");
        } catch (NotEnabledException ex) {
            Activator.getDefault().error(
                    "Double click action not enabled for this type");
        }
    }
    ...
    final class ConnectionOpenRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                Activator.getDefault().error("Unable to build JMX node", ioe);
            } finally {
                // remove the old one.
    ...
    public void showConnectionConfigurationInViewer(
        ...
        try {
            ...
            Activator.getDefault().error("Unable to initializa JMX", ioe);
        }
