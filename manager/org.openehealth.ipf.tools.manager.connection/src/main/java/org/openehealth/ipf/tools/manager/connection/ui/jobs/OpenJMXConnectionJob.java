 ...
 */
package org.openehealth.ipf.tools.manager.connection.ui.jobs;
    ...
    public IStatus run(IProgressMonitor monitor) {
        ...
        try {
            ...
            Activator.getDefault()
                    .info(
                            "OpenJob started for connection "
                                    + connectionConfiguration);
            jMXConnectionManager.openJMXConnection(connectionConfiguration);
            return new Status(Status.OK, Activator.PLUGIN_ID, errorMessage);
        } catch (AuthenticationException ae) {
            Activator.getDefault().warn(
                    "OpenJob failed with AuthenticationException for connection: "
                            + connectionConfiguration);
            errorMessage = Messages
                    .getLabelString(openConnectionCredentialsFailedResourceKey);
        } catch (Throwable ioe) {
            Activator.getDefault().warn(
                    "OpenJob failed with Throwable for connection: "
                            + connectionConfiguration, ioe);
            errorMessage = Messages
